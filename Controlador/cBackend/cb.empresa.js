...
routerEmpresa.route("/empresas")
  ...
  .put(upload.any(),function(req,res){
    modeloEmpresa.findByIdAndUpdate(req.query.id, req.body)
    .then((empresa) =>{
...
routerEmpresa.route("/empresas-adicion-config")
  .put(upload.any(),function(req,res){
    modeloEmpresa.findByIdAndUpdate(req.query.id, req.body)
    .then((empresa) =>{
      res.status(200).send(empresa);
  ...
  routerEmpresa.route("/empresas-contrato-usuario")
    .get(upload.any(),function(req,res){
      modeloEmpresa.findByIdAndUpdate(req.query.id, req.body)
      .then((empresa) =>{
        res.status(200).send(empresa);
      })
